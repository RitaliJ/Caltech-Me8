...
while True:
    ...
    # needs to be changed to absolute value if used
    # if vcmd.any() > vmax.any():
        # print("Exceeding max vel!")
    ...
    if t + dt > tf:
        ...
        match segment_num:
            case 2:
                pf = np.array([pi/3, 0.0])
                tm = movetime(p0, pf, vmax)
            case 3:
                pf = np.array([-pi/3, 0.0])
                tm = movetime(p0, pf, vmax)
            case 4:
                pf = np.array([pi/3, pi/4])
                tm = movetime(p0, pf, vmax)
            case 5:
                pf = np.array([0.0, -pi/6])
                tm = movetime(p0, pf, vmax)
            case 6:
                pf = np.array([-pi/4, pi/6])
                tm = movetime(p0, pf, vmax)
            case 7:
                pf = np.array([0.0, 0.0])
                tm = movetime(p0, pf, vmax)
