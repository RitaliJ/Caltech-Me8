...
while True:
    ...
    if len(faces) > 0:
        ...
        # orig_box_colors = frame[y:y+h-1, x:x+w-1, 0:3]
        # new_box = (orig_box_colors[:,:,2], orig_box_colors[:,:,1], orig_box_colors[:,:,0])
        half = w//2
        if w % 2 == 0:
            temp = frame[y:y+h, x:x+half, :].copy()
            frame[y:y+h, x:x+half, :] = frame[y:y+h, x+w-1:x+half-1:-1, :]
            frame[y:y+h, x+w-1:x+half-1:-1, :] = temp
        else:
            temp = frame[y:y+h, x:x+half, :].copy()
            frame[y:y+h, x:x+half, :] = frame[y:y+h, x+w-1:x+half:-1, :]
            frame[y:y+h, x+w-1:x+half:-1, :] = temp
    ...
    # frame[480-150:480-50, 50:150, 0:3] = (0, 70, 255)
