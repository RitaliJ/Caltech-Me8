...
def controller(shared):
    ...
    key_positions = {'a': np.array([pi/12, 0.0]), \
    'b': np.array([-pi/3, 0.0]), \
    ...
    while True:
        ...
        if shared.lock.acquire():
            ...
            if obj_newdat:
                ...
                pf = np.array([shared.objectpan, shared.objecttilt]) - offset
        ...
        if index < N:
            ...
            if shared.lock.acquire():
                ObjAngles[0,index] = shared.objectpan
                ObjAngles[1,index] = shared.objecttilt
                shared.lock.release()
    ...
    ax5.plot(Time[0:index], ObjAngles[0, 0:index], color='red', linestyle='--', label='Object Pan Angles')
    ax6.plot(Time[0:index], ObjAngles[1, 0:index], color='red', linestyle='--', label='Object Tilt Angles')
    ...
    ax5.set_ylabel('Object Pan Angle (rad)')
    ax6.set_ylabel('Object Tilt Angle (rad)')
